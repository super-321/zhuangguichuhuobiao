...
//        QSocket.receive()
    ...
    /**
     * 下载数据时不需要下载临时回数，因为临时回数只是在PDA里创建和记录的，所使用的条码库与正常回数是同一个。但临时回数要上传到电脑
     */
...
//            QSocket.receive(host, this, showProgress)
            QSocket.receive2(host, this, showProgress) //在 showProgress() 里将接收到的数据写入数据库。runBlock{}。
        } catch (e: Exception) {
            Log.e("q", e.message)
        }
    }
